...
xử lý theo múi giờ Pacific Time (America/Los_Angeles), xuất ra 2 bảng Master:
  ...
  Profit_Phase2_summary_order_items  — chi tiết theo đơn hàng (CSV Order Items mẫu)
  Profit_Phase2_summary_products     — 31 chỉ số hiệu suất theo (ASIN, SKU)
...
API (Profit_Phase1_fin_item_fees, match theo order_id + sku). COGS FIFO theo
effective_date (config_cogs.py).

Thuật toán phân bổ Ad Spend 3 tầng (cho từng kênh SP/SB/SBV/SD):
  Tầng 1: bản ghi cấp SKU/ASIN (Profit_Phase1_ads_sp_asin_daily) -> gán thẳng 100%.
...
# Profit_Phase2_summary_order_items/Profit_Phase2_summary_products có owner_id NOT NULL trong PK
...
#   Rate cache: ƯU TIÊN Profit_Phase1_fee_cache (user override) -> AUTO-DERIVE từ phí thật
...
def calibrate_fee_cache(sb) -> dict:
    """B1 — HỌC profile phí per-SKU từ phí THẬT đã settle, ghi Profit_Phase1_fee_cache:
      fba_fulfillment_fee = median(|FBA|/qty)
      referral_rate       = median(|commission| / (đơn_giá × qty))  [dùng Profit_Phase1_product_price]
    ...
    if not fees:
        print("⚠️  [calibrate] Chưa có phí thật trong Profit_Phase1_fin_item_fees — bỏ qua.")
        return {"calibrated": 0}
    ...
    print(f"✅ [calibrate] {len(rows)} SKU vào Profit_Phase1_fee_cache "
...
def _build_price_map(sb, items: list[dict]) -> dict[str, float]:
    ...
    Phủ chồng theo độ tin cậy TĂNG DẦN (nguồn sau ghi đè nguồn trước):
      ...
      2) Profit_Phase1_product_price (persistent — Phase 1 tự lưu từ MỌI đơn Shipped, không bị --fresh xóa)
    ...
    try:                                               # (2) Profit_Phase1_product_price (persistent)
        for r in fetch_all(lambda: sb.table(T_PRICE).select("sku,unit_price")):
...
# ══════════════════════════════════════════════════════════════════════════════
# 1b. Reimbursements / "Money Back" (AdjustmentEventList — Profit_Phase1_fin_adjustments)
# ══════════════════════════════════════════════════════════════════════════════
...
def _build_reimbursements(adjustments: list[dict], period_start: str, period_end: str,
    ...
    """Gộp Profit_Phase1_fin_adjustments theo (adjustment_type, asin, sku) cho cả kỳ ->
    Profit_Phase2_summary_reimbursements ("Money Back" / Lost & Damaged kiểu Sellerboard)."""
...
def _fetch_ads_by_channel(sb, start_d: str, end_d: str) -> dict[str, list[dict]]:
    ...
    channel: sponsored_products | sponsored_brands | sponsored_brands_video |
             sponsored_display.
    SP ưu tiên bảng cấp SKU (Profit_Phase1_ads_sp_asin_daily) — Tầng 1; thiếu thì
...
# ══════════════════════════════════════════════════════════════════════════════
# 2b. Mart 3 — Campaign Profitability (Profit_Phase2_summary_campaigns)
# ══════════════════════════════════════════════════════════════════════════════

def _build_campaigns(sb, prod: dict, period_start: str, period_end: str) -> list[dict]:
    """Gom Profit_Phase1_ads_campaigns_daily theo campaign + tính lợi nhuận per campaign.

    Profit = GPU(SKU quảng cáo) × units_attributed + ad_spend(âm), trong đó
    GPU (Gross Profit per Unit) lấy từ Mart 2 (prod):
      - Campaign CÓ map SKU (Profit_Phase1_ads_sp_asin_daily — Sponsored Products):
...
def transform(start_local: datetime, end_local: datetime, sb=None) -> dict:
    ...
    # khoá Profit_Phase2_summary_products vì period_start/period_end khác nhau).
    ...
    return {
        ...
        "campaign_rows": campaign_rows,    # Mart 3 — Profit_Phase2_summary_campaigns
        "reimbursement_rows": reimbursement_rows,  # Mart 4 — Profit_Phase2_summary_reimbursements
...
def write_summaries(sb, result: dict) -> dict:
    ...
    # owner_id là 1 phần PRIMARY KEY của Profit_Phase2_summary_order_items/Profit_Phase2_summary_products/
    # Profit_Phase2_summary_reimbursements
...
def main() -> int:
    ...
    ap.add_argument("--calibrate", action="store_true",
                    help="HỌC lại Profit_Phase1_fee_cache (referral_rate + fba_fee per-SKU) từ phí thật "
