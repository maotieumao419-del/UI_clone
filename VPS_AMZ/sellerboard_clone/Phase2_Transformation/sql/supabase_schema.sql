...
CREATE TABLE IF NOT EXISTS "NEW_fin_item_fees" (
    ...
    -- KHÔNG FK -> NEW_sp_orders: Finances lọc theo posted_date, Orders lọc
    -- theo purchase_date -> 2 cửa sổ ngày độc lập, order_id có thể chưa
    -- tồn tại ở NEW_sp_orders tại thời điểm ingest.
...
ALTER TABLE "NEW_fin_item_fees" ADD COLUMN IF NOT EXISTS principal NUMERIC(10,2) DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS "NEW_fin_refunds" (
    id                      BIGSERIAL PRIMARY KEY,
    order_id                TEXT NOT NULL,
    -- KHÔNG FK -> NEW_sp_orders (xem giải thích ở NEW_fin_item_fees)
    posted_date             TIMESTAMPTZ NOT NULL,
    asin                    TEXT,
    sku                     TEXT,
...
-- ============================================================
-- BẢNG 7: NEW_ads_campaigns_daily
...
CREATE INDEX IF NOT EXISTS "idx_NEW_ads_sp_asin_daily_date"
    ON "NEW_ads_sp_asin_daily" (report_date);
CREATE INDEX IF NOT EXISTS "idx_NEW_ads_sp_asin_daily_sku"
    ON "NEW_ads_sp_asin_daily" (advertised_sku);


-- ============================================================
-- BẢNG 9: NEW_product_cogs
...
ALTER TABLE "NEW_fee_cache" ADD COLUMN IF NOT EXISTS sample_count INTEGER DEFAULT 0;

CREATE INDEX IF NOT EXISTS "idx_NEW_fee_cache_sku" ON "NEW_fee_cache"(sku);


-- ============================================================
-- BẢNG 12: NEW_summary_order_items
...
CREATE INDEX IF NOT EXISTS "idx_NEW_summary_order_items_sku"
    ON "NEW_summary_order_items" (sku);


-- ============================================================
-- BẢNG 13: NEW_summary_products
...
CREATE INDEX IF NOT EXISTS "idx_NEW_summary_products_sku"
    ON "NEW_summary_products" (sku);
CREATE INDEX IF NOT EXISTS "idx_NEW_summary_products_period"
    ON "NEW_summary_products" (period_start, period_end);


-- ============================================================
-- BẢNG 14: NEW_summary_campaigns
-- Bảng hiệu quả + lợi nhuận theo từng Campaign quảng cáo
-- ============================================================
CREATE TABLE IF NOT EXISTS "NEW_summary_campaigns" (
    period_start      date         not null,
    period_end        date         not null,
    campaign_id       text         not null,
    campaign_name     text,
...
COMMENT ON TABLE "NEW_summary_campaigns" IS
...
-- ============================================================
-- VIEW 2: NEW_v_daily_sales_localized
-- Daily Sales theo ngày Pacific (UTC -> America/Los_Angeles)
-- ============================================================
CREATE OR REPLACE VIEW "NEW_v_daily_sales_localized" AS
SELECT
...
-- ============================================================
-- VIEW 3: NEW_v_daily_refunds_localized
-- Refund theo ngày Pacific (posted_date)
-- ============================================================
CREATE OR REPLACE VIEW "NEW_v_daily_refunds_localized" AS
SELECT
...
-- ============================================================
DROP FUNCTION IF EXISTS "NEW_fn_daily_summary"(date);
